...
\section{Coefficient of Determination}\label{sec:COD}
    ...
    \end{equation}
    ranging from $\tte[\mi{L}]{\rv{y_0}}$ to $\tte[\mi{L}]{\rv{y_M}}$. Every SP depends stochastically on the ungovernable noise dimension $\tte[M]{\rv{u}} \perp \tte[\mi{M}]{\rv{u}}$ and deterministically on the first $m$ governed inputs $\te[\mi{m}]{u}$, marginalizing the remaining inputs $\tte[\mi{M-m}]{\rv{u}}$. 
    ...
    The remainder of this paper is devoted to calculating these two quantities -- the coefficient of determination and its variance over ungovernable noise (i.e. measurement error, squared).
...
\section{Stochastic Process Estimates}\label{sec:SPEst}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    exploiting the fact that $V_{\mi{0}} = \te[\mi{L}^2]{0}$. \Cref{eq:SPEstimates:reduction} cancels all terms beginning with $\te[\mi{L}]{\rv{e_{m}}}^{2}$, first across $A_{\mi{mm^{\prime}}}-A_{\mi{0m^{\prime}}}$ then across $A_{\mi{m0}}-A_{\mi{00}}$. All remaining terms ending in $\te[\mi{L}]{\mu_{\mi{m^{\prime}}}}^{2}$ are eliminated by centralization $\evt{\,\tte[]{\rv{e_{m}}}}{M} = 0$ and
    \begin{multline*}
...
\section{Interlude: Gaussian Process Regression} \label{sec:GPR}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    supporting the fundamental definition of the GP kernel, as a covariance (over ungovernable noise) between responses
    \begin{equation*}
    ...
    \subsection{Prior GP} \label{sub:GPR:Prior}
        GP regression decomposes output $\te[\mi{L}]{\rv{y_M}}$ into signal GP $\te[\mi{L}]{\rv{f_M}}$, and independent noise GP $\te[\mi{L}]{\rv{e}_M}$ with homoskedastic noise covariance $\te[\mi{L}^2]{E}$
        \begin{equation*}
            \begin{aligned}
    ...
    \subsection{Predictive GP} \label{sub:GPR:Predictive}
        Bayesian inference for GP regression further conditions the hyper-parametrized GP $\rv{y} \vert E,F,\Lambda$ on the observed realization (over ungovernable noise) of the random variable $\te{\rv{y}\vert X}$
        \begin{equation*}
        ...
        The second moment of the GP for any $\mi{m,m^{\prime}}\subseteq\mi{M}$ is given by
            ...
            \begin{equation*}
                \begin{aligned}
                    ...
                    \te[l\x l^{\prime}\x\mi{m}\x\mi{M}]{\Phi}
                    \diag[l\x l^{\prime}\x\mi{M}\x\mi{M^{\prime}}]{\Lambda^{2}+2}^{-1} \te[l\x l^{\prime}\x\mi{m^{\prime\prime}}\x\mi{M^{\prime}}]{\Phi}^{\intercal} \\
...
\end{align*}
    ...
    \subsection{Variance}\label{sub:GPEst:Variance}
        ...
        \end{gather*}
        This reduces the largest tensor $\tte[]{H_{\mi{m}}}$ needed to calculate a Sobol' index $S_{\mi{m}}$ from $L^{4}N^{2}$ dimensions to $L^{2}N^{2}$. The same reduction factor $L^{2}$ applies to the two largest terms needed to calculate the variance $T_{\mi{m}}$. Note that the noise variance $\te[\mi{L}^{2}]{E}$ need not be diagonal to achieve these reductions.
        ...
        \end{equation*}
        This reduces the largest tensor in the calculation of $T_{\mi{m}}$ from $L^{4}N^{3}$ dimensions to $L^{2}N^{3}$, or from $L^{2}N^{3}$ to $LN^{3}$ if the first simplification has already been applied.


\section{Conclusion}\label{sec:Conc}
    In this paper, we transformed uniformly distributed inputs $u$ to normally distributed inputs $z$, and arbitrarily roated them, prior to fitting a Multi-Output Gaussian Process (MOGP). This is restricted to an anisotropic radial basis function (RBF/ARD) kernel, broadly applicable to smoothly varying outputs. Using this surrogate, analytic expressions for closed Sobol' indices $S_{\mi{m}}$ are given by \cref{def:COD:mean,eq:SPEst:V} and \cref{sub:GPEst:Expectation}. Analytic expressions for the variance of these estimates over ungovernable noise is given by \cref{def:COD:variance,eq:SPEst:A,eq:SPEst:W} and \cref{sub:GPEst:Variance}. Cheap simplifications of the results are described in \cref{sub:GPEst:Simp}.

    The use of a surrogate should greatly ease data requirements, preliminary tests indicating that 100 datapoints is sufficient for Sobol' indices within $10\%$ accuracy, as opposed to 10,000+ datapoints for direct calculation. However, we would caution against allowing more than 15 inputs in any case, as all GPs are extremely susceptible to every aspect of the curse of dimensionality
    Perhaps the most significant feature of this work is the natural cancellation of the regression noise $E$ from the calculation of Sobol' indices, and their variances.

    This allows for an arbitrary rotation $\Theta$ of inputs, as normal variables are additive, whereas summing uniform inputs does not produce uniform inputs. If the goal is reducing inputs, rotating their basis first boosts the possibilities immensely \cite{Constantine2015}. It presents the possibilty of choosing $\Theta$ to maximise the closed Sobol' index of the first few inputs.
