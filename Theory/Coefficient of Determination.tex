...
\section{Generalized Sobol' indices}\label{sec:GSI}
    ...
    \end{align}
    with variance due to ungoverned noise of
    \begin{equation} \label{def:GSI:variance}
        \te[(\mi{L\x L^{\prime}})^2]{T_{\mi{m}}\circ T_{\mi{m}}} \deq 
    ...
    \begin{equation} \label{eq:GSI:variance:diagonal}
        \te[l^{4}]{T_{\mi{M}}}=0 \QT{on the diagonal} \te[l^{2}]{\rv{S_M}}=1        
    \end{equation}
    ...
    In a similar vein, the total index is defined as the difference between the closed index and $\tte[\mi{L\x L^{\prime}}]{\rv{S_M}}$, which is exactly 1 on the diagonal and usually highly correlated with $\tte[\mi{L\x L^{\prime}}]{\rv{S_m}}$ off the diagonal. Tiny differences between terms in the correlated case are swamped by numerical errors in practice. The conservative standard error
    \begin{equation} \label{def:GSI:total:variance}
        \begin{aligned}
            \te[(l\x l^{\prime})^2]{T_{\mi{M-m}}^{T}} &\deq 
            \sqrt{\te[(l\x l^{\prime})^2]{T_{\mi{M}}\circ T_{\mi{M}}}} + \sqrt{\te[(l\x l^{\prime})^2]{T_{\mi{m}}\circ T_{\mi{m}}}}\\
            &\geq \sqrt{\covt{\,\te[(l\x l^{\prime})^2]{\rv{S}_{\mi{M-m}}^{T}}}{M}}
        \end{aligned}
    \end{equation}
    achieves equality on the diagonal, and is robust and sufficiently precise for most practical purposes.

    The remainder of this paper is devoted to calculating these two quantities -- the generalized closed Sobol' Index $S_{\mi{m}}$ and its standard error due to ungoverned noise $T_{\mi{m}}$.
...
\section{Gaussian Process estimates}\label{sec:GPEst}
    Using the work of the last two Sections, we are finally in a position to calculate Gaussian process estimates for the Sobol' indices \cref{def:GSI:mean} and their standard error \cref{def:GSI:variance} via the two unknown quantities in \cref{eq:SPEst:V} and \cref{eq:SPEst:W}, as described in \cref{sec:SPEst}.
...
\section{Complexity and simplifications}\label{sec:Complexity}
    ...
    \end{equation*}
    From \cref{sub:GPEst:Variance} the memory required to compute a standard error $T_{\mi{m}}$ is
    \begin{equation*}
    ...
    The first simplification observes that assessing standar error concerns the variances of Sobol' indices, not the cross covariances between them. This costlessy reduces the tensor elements which must be calculated according to
    \begin{equation*}
    ...
    \end{equation*}
    The memory required to compute a standard error $T_{\mi{m}}$ is reduced by $O(L)$ to
    \begin{equation*}
    ...
    The second simplification is to restrict the GP to independent kernels, by constraining the signal covariance to be diagonal 
    \begin{equation*}
