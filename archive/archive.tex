...
\begin{document}
    ...
    %     \subsection{Conditional Covariance Error} \label{sub:GSA:CovarianceError}


            % \subsection{Conditional Covariance} \label{sub:ROM:Covariance}
        %     Using Fubini's theorem, the ($\mi{m}$) conditional covariance is
        %     \begin{equation*} \label{def:ROM:Covariance:sigma}
        %         \begin{aligned}
        %             \te[\mi{L}\x\mi{L}]{V\!\left(\te[\mi{m}\x\mi{M}]{\Theta}\right)} 
        %             &\deq \ev[\mi{L}]{\;}\var[\mi{m}]
        %             {\left\lbrack\ev[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack} \\
        %             &= \ev[\mi{L}]{\;}\ev[\mi{m}]
        %             {\left\lbrack\evk[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}{2}\right\rbrack\right\rbrack} 
        %             - \ev[\mi{L}]{\;}\evk[\mi{m}]
        %             {\left\lbrack\ev[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack}{2} \\
        %             &= \ev[\mi{m}]{\;}\ev[\mi{L}]
        %             {\left\lbrack\evk[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}{2}\right\rbrack\right\rbrack} 
        %             - \ev[\mi{L}]
        %             {\left\lbrack\evk[\mi{M}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}{2}\right\rbrack\right\rbrack} \\
        %             &= \ev[\mi{m}]{\;}\var[\mi{L}]
        %             {\left\lbrack\ev[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack} 
        %             - \var[\mi{L}]
        %             {\left\lbrack\ev[\mi{M}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack} \\
        %             &\phantom{\deq\ } + \ev[\mi{m}]{\;}\evk[\mi{L}]
        %             {\left\lbrack\ev[\mi{M-m}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack}{2} 
        %             - \evk[\mi{L}]
        %             {\left\lbrack\ev[\mi{M}]{\left\lbrack\te[\mi{L}]{\rv{y} \vert Y}}\right\rbrack\right\rbrack}{2} \\
        %             &= \ev[\mi{m}]{\te[\mi{L}\x\mi{L^{\prime}}]{\sigma\!\left(\te[\mi{m}]{\rv{z}}; \te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)}}
        %              - \te[\mi{L}\x\mi{L}]{\sigma\!\left(\right)} \\
        %             &\phantom{\deq\ } + \ev[\mi{m}]{\left\lbrack
        %             \te[\mi{L}]{f\!\left(\te[\mi{m}]{\rv{z}}; \te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)}^2 \right\rbrack}
        %              - \te[\mi{L}]{f\!\left(\right)}^2 \\
        %              &= \ev[\mi{m}]{\left\lbrack
        %             \te[\mi{L}]{f\!\left(\te[\mi{m}]{\rv{z}}; \te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)}^2 \right\rbrack}
        %              - \te[\mi{L}]{f\!\left(\right)}^2
        %         \end{aligned}
        ...
        %     Using
        %     $\tte[l\x\mi{L^{\prime\prime}}\x\mi{N^{\prime\prime}}]{K_{Y}^{-1} Y^{\dagger} \circ \te{G}^{\dagger}} \deq K_{Y}^{-1} \te[\mi{L^{\prime\prime}}\x\mi{N^{\prime\prime}}]{Y}^{\dagger} \circ \te[l\x\mi{L^{\prime\prime}}\x 1\x\mi{N^{\prime\prime}}]{G}^{\dagger}$
        %     to write
        %     \begin{multline*}                
        %         \ev[\mi{m}]{\te[l \x l^{\prime}]{
        %         \te[\mi{L}]{f\!\left(\te[\mi{m}]{\rv{z}}; \te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)}^2}}
        %         = \te[l\x\mi{L^{\prime\prime}}\x\mi{N^{\prime\prime}}]{K_{Y}^{-1} Y^{\dagger} \circ \te{G}^{\dagger}} \\
        %         \te[l \x l^{\prime}\x\mi{L^{\prime\prime}}\x\mi{L^{\prime\prime\prime}}\x\mi{N^{\prime\prime}}\x\mi{N^{\prime\prime\prime}}]{H\!\left(\te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)}^{\dagger}
        %         \te[l^{\prime}\x\mi{L^{\prime\prime\prime}}\x\mi{N^{\prime\prime\prime}}]{K_{Y}^{-1} Y^{\dagger} \circ \te{G}^{\dagger}}
        %     \end{multline*}
        %     results in
        %     \begin{multline*}
        %         \te[l \x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x n^{\prime\prime}\x n^{\prime\prime\prime}]{H\!\left(\te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)} \deq \\
        %         \ev[\mi{m}]{\te{\frac{
        %             \prob{\te[\mi{m}]{\rv{z}}}{\te[\mi{m}\x l\x l^{\prime\prime}\x n^{\prime\prime}]{G}}{\te[l\x l^{\prime\prime}\x\mi{m}\x\mi{m}]{\Gamma}}
        %             \prob{\te[\mi{m}]{\rv{z}}}{\te[\mi{m}\x l^{\prime}\x l^{\prime\prime\prime}\x n^{\prime\prime\prime}]{G}}{\te[l^{\prime}\x l^{\prime\prime\prime}\x \mi{m}\x\mi{m}]{\Gamma}}}
        %         {\prob{\te[\mi{m}]{\rv{z}}}{\te[\mi{m}]{0}}{\diag[\mi{m\x m}]{1}}
        ...
        %     \end{multline*}
        %     Using \cref{eq:Notation:product} twice, with Hadamard (element-wise) division and product
        %     \begin{multline*} \label{def:ROM:Expectation:f}
        %         \te[l \x l^{\prime}\x\mi{L^{\prime\prime}}\x\mi{L^{\prime\prime\prime}}\x\mi{N^{\prime\prime}}\x\mi{N^{\prime\prime\prime}}]{H\!\left(\te[\mi{m}\x\mi{M^{\prime}}]{\Theta}\right)} \deq
        %         \te[l\x l^{\prime}\x \mi{L^{\prime\prime}}\x \mi{L^{\prime\prime\prime}}]{\modulus{\Psi}^{-1}} \circ \\
        %         \frac{
        %             \prob{\te[\mi{m}\x l^{\prime}\x\mi{L^{\prime\prime\prime}}\x\mi{N^{\prime\prime\prime}}]{G}}
        %             {\te[\mi{m}\x l\x\mi{L^{\prime\prime}}\x\mi{N^{\prime\prime}}]{G}}{\te[l \x l^{\prime}\x\mi{L^{\prime\prime}}\x\mi{L^{\prime\prime\prime}}\x\mi{m}\x\mi{m}]{\Sigma}}
        ...
        %     \end{multline*}
        %     where
        %     \begin{equation*} \label{def:ROM:Expectation:USigma}
        %         \begin{aligned}
        %             \te[l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x\mi{m}\x\mi{m}]{\Sigma} &\deq 
        %             \te[l\x l^{\prime\prime}\x\mi{m}\x\mi{m}]{\Gamma} + \te[l^{\prime}\x l^{\prime\prime\prime}\x\mi{m}\x\mi{m}]{\Gamma} \\
        %             \te[l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x\mi{m}\x\mi{m^{\prime}}]{\Psi} &\deq 
        %             \te[l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x\mi{m}\x\mi{m^{\prime}}]{\Sigma} \\
        %             &\phantom{\deq}- \te[l\x l^{\prime\prime}\x\mi{m}\x\mi{m^{\prime\prime}}]{\Gamma} \te[l^{\prime}\x l^{\prime\prime\prime}\x\mi{m^{\prime\prime}}\x\mi{m^{\prime}}]{\Gamma} \\
        %             \te[l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}]{\modulus{\Psi}^{-1}} &\deq 
        %             \modulus{\te[l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x\mi{m^{\prime\prime}}\x\mi{m^{\prime}}]{\Psi}}^{-1} \\
        %             \te[\mi{m}\x l\x l^{\prime}\x l^{\prime\prime}\x l^{\prime\prime\prime}\x\mi{N^{\prime\prime}}\x\mi{N^{\prime\prime\prime}}]{\Sigma G} &\deq 
        %             \te[l^{\prime}\x l^{\prime\prime\prime}\x\mi{m}\x\mi{m^{\prime}}]{\Gamma}
        ...
        %     It is worth noting that the totally conditional covariance is independent of $\Theta$ and should have 1s along the diagonal due to standardization of $Y$, although this may be inexact.
    % \section{Basis Optimization} \label{sec:BasisOptimization}
    ...
    %     \begin{equation*}
    %         \tr{\rv{z}}_{\tr{m}} = (\Theta\tr{x})_{\tr{m}} = (\Theta)_{\tr{m}\x\tr{M}} (\tr{x})_{\tr{M}} 
    %     \end{equation*}
    %     For the purpose of merely calculating the closed Sobol' indices 
    %     \begin{equation*} \label{eq:Notation:BO:SDef}
    %         S_{\tr{m}}((\Theta)_{\tr{m}\x\tr{M}}) \deq \frac{D_{\tr{m}}((\Theta)_{\tr{m}\x\tr{M}})}{D_{\tr{M}}} 
    %     \end{equation*}
    %     we may take $\Theta = (\tr{I})_{\tr{M}\x\tr{M}}$ and cancel it throughout \cref{sub:Notation:GSA}, and our work is complete. 
    %     To locate an active subspace, on the other hand, is to seek
    %     \begin{equation*} \label{eq:Notation:BO:OptimalRow}
    %         (\Theta)_{\tr{m}\x\tr{M}} = \argmax S_{\tr{m}}((\Theta)_{\tr{m}\x\tr{M}}) \QT{such that} S_{\tr{m}} \approx 1
    %     \end{equation*}
    %     This is a fraught and onerous calculation, featuring an infinitude of optima for $m>1$ (corresponding to alternative bases for the same active subspace). It is far more cogent and efficient to focus on the marginalized dimensions $\tr{M} - \tr{m}$ via the total Sobol' index
    %     \begin{equation*} \label{eq:Notation:BO:StotDef}
    %         S_{\tr{M} - \tr{m}}^{T}((\Theta)_{\tr{M} - \tr{m}\x\tr{M}}) \deq 1 - S_{\tr{m}}((\Theta)_{\tr{m}\x\tr{M}}) \deqr 1 - \frac{\tilde{D}_{\tr{m}}((\Theta)_{\tr{M} - \tr{m}\x\tr{M}})}{D_{\tr{M}}}
    %     \end{equation*}
    %     This directly measures the output covariance lost by ignoring the input dimensions in $\tr{M} - \tr{m}$, without fussing over how the output covariance is distributed over the remaining $m$ input dimensions.
    %     This enables fast and efficient optimisation, with the flexibility to make $M-m$ as small as needed, whereas $m$ is bounded below by the number of dimensions needed to explain most of the output covariance.

    %     It is clear from \cref{eq:Notation:GSA:DmDef,eq:Notation:BO:SDef,eq:Notation:BO:StotDef} that
    %     \begin{multline*}
    %         \bar{\tr{f}}_{0} \; \tilde{\tr{D}}((\Theta)_{\tr{M} - \tr{m}\x\tr{M}}) 
    %         \; \bar{\tr{f}}_{0}^{\intercal} - \bar{f}_{\tr{0}}^{2}
    %         \deq \tilde{D}_{\tr{m}}((\Theta)_{\tr{M} - \tr{m}\x\tr{M}}) \\
    %         \deq D_{\tr{m}}((\Theta)_{\tr{m}\x\tr{M}})
    %         = \bar{\tr{f}}_{0} \; \tr{D}((\Theta)_{\tr{m}\x\tr{M}}) 
    %         \; \bar{\tr{f}}_{0}^{\intercal} - \bar{f}_{\tr{0}}^{2}
    %     \end{multline*}
    %     so $\tr{D}((\Theta)_{\tr{m}\x\tr{M}})$ must be expressable in terms of $(\Theta)_{\tr{M} - \tr{m}\x\tr{M}}$ alone.
    %     This is achieved using \cref{eq:Notation:scaling} to write
    %     \begin{equation*} \label{eq:Notation:BO:DtDef}
    %         (\tilde{\tr{D}}((\Theta)_{\tr{M} - \tr{m}\x\tr{M}}))_{n \x o} \deq
    %         \lvert\tilde{\Psi}\rvert^{-1} \frac 
    %         {p\!\left((\tr{0})_{\tr{m}} ; \left((\tilde{\tr{G}})_{\tr{m}\x n}-(\tilde{\tr{G}})_{\tr{m}\x o}\right), 2(\tilde{\Gamma})_{\tr{m}\x\tr{m}}\right)}
    %         {p\!\left((\tr{0})_{\tr{m}} ; \left((\tilde{\tr{G}})_{\tr{m}\x n}+(\tilde{\tr{G}})_{\tr{m}\x o}\right), 2(\tilde{\Psi})_{\tr{m}\x\tr{m}}\right)}
    %     \end{equation*}
    %     where
    %     \begin{equation*} \label{eq:Notation:BO:FbtSigmatDef}
    %         \begin{aligned}
    %             \tilde{\tr{G}} &\deq 
    %                 \tilde{\Theta} \left(\Lambda^{2} + \tr{I}\right)^{-1} \tr{X} \\
    %             \tilde{\Gamma} &\deq 
    %                 \tilde{\Theta} \left(\Lambda^{-2} + \tr{I}\right)^{-1} \tilde{\Theta}^{\intercal} \\
    %             \tilde{\Psi} &\deq \tilde{\Theta} \left(\Lambda^{-2}+\tr{I}\right)^{-1}\left(2\Lambda^{-2}+\tr{I}\right) 
    %             \tilde{\Theta}^{\intercal}
    %         \end{aligned}                    
    %     \end{equation*}
    %     and
    %     \begin{equation*} \label{eq:Notation:BO:ThetatDef}
    %         \tilde{\Theta} \deq (\Theta)^{\intercal}_{\tr{m}\x\tr{m}}(\Theta)_{\tr{m}\x\tr{M}} = 
    %         (\tr{I})_{\tr{m}\x\tr{M}} - (\Theta)^{\intercal}_{\tr{M} - \tr{m}\x\tr{m}}
    %         (\Theta)_{\tr{M} - \tr{m}\x\tr{M}}
    %     \end{equation*}
    %     A sufficient condition for validity is invertibility, which, using Weinstein-Aronszajn \cite{Tao2013}, may be expressed as
    %     \begin{equation*} \label{eq:Notation:BO:Aronszajn}
    %         \modulus{(\Theta)_{\tr{m}\x\tr{m}}}^2 = \modulus{(\Theta)_{\tr{M} - \tr{m}\x\tr{M} - \tr{m}}}^2 > 0
    %     \end{equation*}

    %     Taking $M-m>1$ can potentially eliminate an $M-m$ dimensional space whose influence is even less than the $M-m$ least influential inputs found and eliminated in order. However, this does not seem worth the greatly increased complexity and infinitude of local optima introduced.
    %     We therefore take $M-m=1$ and seek
    %     \begin{equation*} \label{eq:Notation:BO:OptimalRow}
    %         (\Theta)_{M\x\tr{M}} = \argmin S^{T}_{M}((\Theta)_{M\x\tr{M}}) \QT{such that} S^{T}_{M} \approx 0
    %     \end{equation*}
    %     to construct the unique $RQ$ decomposition
    %     \begin{equation*} \label{eq:Notation:BO:RQ}
    %         \tr{R}\Theta = 
    %         \left(\Theta\right)_{\tr{M}\x \tr{M}} \QT{where} \left(\Theta\right)_{m\x \tr{M}} \deq 
    %         \begin{cases}
    %             (\tr{I})_{m\x\tr{M}} \ & m<M \\
    %             (\Theta)_{M\x\tr{M}} \ & m=M \\
    %         \end{cases}
    %     \end{equation*}
    %     for upper triangular $\tr{R}$. By construction, the resulting $\Theta$ is orthogonal (thanks to \cref{eq:Notation:BO:Aronszajn}) and has the final row $(\Theta)_{M\x\tr{M}}$ caclulated by \cref{eq:Notation:BO:OptimalRow}.
    %     \begin{equation*} \label{eq:Notation:BO:derivAdjust}
    %         \begin{aligned}
    %             \T{For } m & \in \tr{M-1} \\
    %             \frac{\partial}{\partial(\Theta)_{M\x m}} 
    %             &= \sum_{k=1}^{M-1} \sum_{l=1}^{M}\frac{\partial (\tilde{\Theta})_{k\x l}}{\partial(\Theta)_{M\x m}} \frac{\partial}{\partial (\tilde{\Theta})_{k\x l}} \\
    %             &= -\sum_{k=1}^{M-1} (\Theta)_{M\x k} \frac{\partial}{\partial (\tilde{\Theta})_{k\x m}} 
    %             - \sum_{l=1}^{M} (\Theta)_{M\x l} \frac{\partial}{\partial (\tilde{\Theta})_{m\x l}} \\
    %             \frac{\partial}{\partial(\Theta)_{M\x M}} 
    %             &= \sum_{k=1}^{M-1} \frac{\partial (\tilde{\Theta})_{k\x M}}{\partial(\Theta)_{M\x M}} \frac{\partial}{\partial (\tilde{\Theta})_{k\x M}} 
    %             = - \sum_{k=1}^{M-1} (\Theta)_{M\x k} \frac{\partial}{\partial (\tilde{\Theta})_{k\x M}} \\
    %         \end{aligned}
    %     \end{equation*}
    %     \begin{equation*} \label{eq:Notation:BO:derivAdjust}
    %         \begin{aligned}
    %             \T{For } m & \in \tr{M-1}  \\
    %             (\Theta)_{M\x M} &= \left(1 - \sum (\Theta)_{M\x m}^{2}\right)^{1/2} \\
    %             \frac{\partial}{\partial(\Theta)_{M\x m}} &= \frac{\partial}{\partial(\Theta)_{M\x m}} - 
    %             \frac{(\Theta)_{M \x m}}{(\Theta)_{M \x M}}\frac{\partial}{\partial(\Theta)_{M\x M}}
    %         \end{aligned}
    %     \end{equation*}
    %     \begin{multline*} \label{eq:Notation:BO:derivAdjust}
    %         \frac{\partial}{\partial(\Theta)_{M\x m}} 
    %         = -(\Theta)_{M\x M} \frac{\partial}{\partial (\tilde{\Theta})_{m\x M}} \\
    %         + \sum_{k=1}^{M-1} (\Theta)_{M\x k} 
    %         \left(\frac{(\Theta)_{M \x m}}{(\Theta)_{M \x M}}\frac{\partial}{\partial(\tilde{\Theta})_{k\x M}}
    %         -\frac{\partial}{\partial (\tilde{\Theta})_{k\x m}}-\frac{\partial}{\partial (\tilde{\Theta})_{m\x k}} \right)
    %     \end{multline*}

    %     \begin{algorithm} \label{al:Notation:BO}
    %     \caption{Summary of the basis optimization algorithm.}
    %         \begin{algorithmic}[1]
    %             \REQUIRE $y(\tr{X}) \colon \st{R}^{M\x N} \rightarrow \st{R}$
    %             \ENSURE $y(\tr{Z}^{\intercal}) \colon \st{R}^{m\x N} \rightarrow \st{R}$ where $m \leq M $

    %             \LOOP
    %                 \STATE Fit GP surrogate to $y(\tr{X})$, determining $\bar{f}(\tr{x})$ according to \cref{sub:GP}
    %                 \STATE Swap input dimensions $M$ and $k\leq M$ to maximise $D_{\tr{M-1}}((\tr{I})_{\tr{M-1}\x\tr{M}})$, calculated according to \cref{sub:Notation:GSA}
    %                 \STATE Optimize
    %                 $ (\Theta)_{M\x\tr{M}} = \argmin S^{T}_{M}((\Theta)_{M\x\tr{M}}) $
    %                 according to \cref{sub:Notation:BO}
    %                 \IF{$S^{T}_{M} \not\approx 0$} 
    %                     \RETURN $y(\tr{X})$
    %                 \ENDIF
    %                 \STATE Set $\Theta$ by $RQ$ decomposition, according to \cref{eq:Notation:BO:RQ}
    %                 \STATE Update the input basis to $\tr{X} \leftarrow \Theta \tr{X}$
