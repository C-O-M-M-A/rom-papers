...
\newcommand*{\gcf}[4][]{\M{\mathrm{P}^{#1}\!\left(\left.{#2}\right\vert{#3,#4}\right)}} 
...
\section{Introduction}\label{sec:Intro}
    ...
    The contents of this paper are as follows. introduces the notation and context for performing GSA in \cite{Milton.etal.2024a}.


\section{Multi-output models with quantified uncertainty (MQUs)}\label{sec:MQU}
    ...
    \begin{equation} \label[definition]{def:MQU:z}
        \te[\mi{M+1}]{z} \deq \gcf[-1]{\te[\mi{M+1}]{u}}{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}} \sim \gauss{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}}
    \end{equation}
    where $\gauss{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}}$ is the $\mi{M+1}$ dimensional standard normal distribution with mean $\te[\mi{M+1}]{0}$ and variance $\diag[\mi{M+1}^{2}]{1}$, and $\gcf[-1]{\te[\mi{M+1}]{u}}{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}}$ is its quantile function, or inverse CDF. Henceforth, angle brackets always denote a diagonal tensor, identically 0 off the leading diagonal of the final two axes. The advantage of normally 
    \begin{equation} \label[definition]{def:MQU:z}
        \te[\mi{M+1}]{z} \deq \gcf[-1]{\te[\mi{M+1}]{u}}{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}} \sim \gauss{\te[\mi{M+1}]{0}}{\diag[\mi{M+1}^{2}]{1}}
    \end{equation}
